...
const Instruction = moonlet.opcodes.Instruction;
const Node = moonlet.ast.Node;
const Function = moonlet.object.Function;
const Constant = moonlet.object.Constant;
const Lexer = moonlet.lex.Lexer;
const Parser = moonlet.parse.Parser;
const max_stack_size = moonlet.parse.max_stack_size;
const OpCode = moonlet.opcodes.OpCode;
const Token = moonlet.lex.Token;
...
pub const Compiler = struct {
    ...
    pub const Func = struct {
        ...
        pub fn freereg(self: *Func, reg: u9) void {
            if (!moonlet.opcodes.rkIsConstant(reg) and reg >= self.num_active_local_vars) {
        ...
        pub fn exp2RK(self: *Func, e: *ExpDesc) !u9 {
            ...
            switch (e.desc) {
                .number, .true, .false, .nil => {
                    if (self.constants.items.len <= moonlet.opcodes.rk_max_constant_index) {
                        ...
                        return moonlet.opcodes.constantIndexToRK(@intCast(index));
                    }
                },
                .constant_index => {
                    if (e.desc.constant_index <= moonlet.opcodes.rk_max_constant_index) {
                        return moonlet.opcodes.constantIndexToRK(@intCast(e.desc.constant_index));
                    }
                },
                else => {},
            }
    ...
    pub fn genNode(self: *Compiler, node: *Node) Error!void {
        switch (node.id) {
            ...
            .call => try self.genCall(@alignCast(@fieldParentPtr("base", node))),
            .assignment_statement => try self.genAssignmentStatement(@alignCast(@fieldParentPtr("base", node))),
            .literal => try self.genLiteral(@alignCast(@fieldParentPtr("base", node))),
            .identifier => try self.genIdentifier(@alignCast(@fieldParentPtr("base", node))),
            .return_statement => try self.genReturnStatement(@alignCast(@fieldParentPtr("base", node))),
            .field_access => try self.genFieldAccess(@alignCast(@fieldParentPtr("base", node))),
            .index_access => try self.genIndexAccess(@alignCast(@fieldParentPtr("base", node))),
            .table_constructor => try self.genTableConstructor(@alignCast(@fieldParentPtr("base", node))),
            .table_field => unreachable, // should never be called outside of genTableConstructor
            .binary_expression => try self.genBinaryExpression(@alignCast(@fieldParentPtr("base", node))),
            .grouped_expression => try self.genGroupedExpression(@alignCast(@fieldParentPtr("base", node))),
            .unary_expression => try self.genUnaryExpression(@alignCast(@fieldParentPtr("base", node))),
            inline else => |id| @panic("id: " ++ @tagName(id) ++ " not implemented"), // TODO
        }
    }
    ...
    pub fn genTableConstructor(self: *Compiler, table_constructor: *Node.TableConstructor) Error!void {
        ...
        var num_keyed_values: moonlet.object.FloatingPointByteIntType = 0;
        var num_array_values: moonlet.object.FloatingPointByteIntType = 0;
    ...
    pub fn genLiteral(self: *Compiler, literal: *Node.Literal) Error!void {
        switch (literal.token.id) {
            .string => {
                ...
                const parsed = moonlet.parse_literal.parseString(string_source, buf);
            ...
            .number => {
                ...
                const parsed = moonlet.parse_literal.parseNumber(number_source);
...
fn testCompile(source: [:0]const u8) !void {
    ...
    try moonlet.debug.checkcode(&chunk);

    var buf = std.ArrayList(u8).init(std.testing.allocator);
    defer buf.deinit();
    try moonlet.dump.write(chunk, buf.writer());
