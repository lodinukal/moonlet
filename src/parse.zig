...
const LexErrorContext = moonlet.lex.LexErrorContext;
...
pub const ParseErrorContext = struct {
    ...
    pub fn renderAlloc(self: *ParseErrorContext, allocator: Allocator, parser: *Parser) ![]const u8 {
        ...
        const chunk_id = moonlet.object.getChunkId(parser.lexer.chunk_name, &chunk_id_buf);
...
pub const Parser = struct {
    ...
    fn exprstat(self: *Self) Error!*Node {
        ...
        if (expression.node.id == .call) {
            const call_node: *align(1) Node.Call = @fieldParentPtr("base", expression.node);
    ...
    // :moonlet changes
    /// parses a type
    fn anyty(self: *Self) Error!*Node {
        switch (self.state.token.id) {
            .string => {
                const string_singleton = try self.state.arena.create(Node.Type);
                string_singleton.* = .{
                    .kind = .{ .string = true },
                    .start_token = self.state.token,
                };
                try self.nextToken();
                return &string_singleton.base;
            },
            .number => {
                const number_singleton = try self.state.arena.create(Node.Type);
                number_singleton.* = .{
                    .kind = .{ .number = .f64 },
                    .start_token = self.state.token,
                };
                try self.nextToken();
                return &number_singleton.base;
            },
            .name => {
                return self.namety();
            },
            .keyword_true => {
                const boolean_singleton = try self.state.arena.create(Node.Type);
                boolean_singleton.* = .{
                    .kind = .{ .boolean = true },
                    .start_token = self.state.token,
                };
            ...
            .single_char => {
                if (self.state.token.char == '(') {
                    ...
                    if (self.state.token.id == .skinny_arrow) {
                        try self.nextToken();
                        const returns = try self.anyty();
                        const param_type = maybe_function_params.cast(.type).?;
                        const return_type = returns.cast(.type).?;
                        const function = try self.state.arena.create(Node.Type);
                        function.* = .{
                            .kind = .{ .function = .{
                                .params = param_type,
                                .returns = return_type,
                            } },
                            .start_token = param_type.start_token,
                        };
                        return &function.base;
                    }
                    return maybe_function_params;
                } else {
                    std.log.info("got: {?c}", .{self.state.token.char});
                }
    ...
    const Builtin = enum {
        ...
        pub fn toAst(builtin: Builtin, start_token: Token) Node.Type {
            return switch (builtin) {
                ...
                .i32, .i64, .f32, .f64 => |sub_builtin| .{
                    .kind = .{
                        .number = switch (sub_builtin) {
                            .i32 => .i32,
                            .i64 => .i64,
                            .f32 => .f32,
                            .f64 => .f64,
    ...
    fn namety(self: *Self) Error!*Node {
        const start_token = self.state.token;
        // check beforehand if its a builtin type
        if (start_token.id == .name) {
            if (try self.tokenty(start_token)) |ty| {
                try self.nextToken();
                return ty;
            }
        }

        const ty = try self.state.arena.create(Node.Type);
        ty.* = .{
            .kind = .{ .expression = try self.expr() },
            .start_token = start_token,
        };
        return &ty.base;
    }

    /// makes a type from a token
    fn tokenty(self: *Self, token: Token) Error!?*Node {
        const found_builtin = std.meta.stringToEnum(Builtin, self.tokenstr(token));
        if (found_builtin) |fb| {
            const ty = try self.state.arena.create(Node.Type);
            ty.* = fb.toAst(token);
            return &ty.base;
    ...
    fn tuplety(self: *Self) Error!*Node {
        const start_token = self.state.token;
        std.debug.assert(self.state.token.isChar('('));
        try self.nextToken();
        ...
        var types = std.ArrayList(*Node.Type).init(self.state.allocator);
        errdefer types.deinit();

        while (!self.state.token.isChar(')')) {
            const lookahead = try self.lexer.lookahead();
            if (lookahead.isChar(':')) {
                const name = try self.optcheckname();
    ...
    fn tokenstr(self: *Self, token: Token) []const u8 {
        return self.lexer.buffer[token.start..token.end];
    }
    ...
    // :moon changes
    fn optcheckname(self: *Self) !?Token {
        if (self.state.token.id == .name) {
            const token = self.state.token;
            try self.nextToken();
            return token;
