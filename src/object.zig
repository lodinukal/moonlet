...
pub const Function = struct {
    ...
    pub fn printCode(self: *const Function) void {
            ...
            switch (op.getOpMode()) {
                .iABC => {
                    ...
                    if (op.getBMode() != .NotUsed) {
                        const b_for_display: i32 = if (moonlet.opcodes.rkIsConstant(@intCast(b)))
                            (-1 - @as(i32, @intCast(moonlet.opcodes.rkGetConstantIndex(@intCast(b)))))
                        else
                            b;
                        std.debug.print(" {d}", .{b_for_display});
                    }
                    if (op.getCMode() != .NotUsed) {
                        const c_for_display: i32 = if (moonlet.opcodes.rkIsConstant(@intCast(c)))
                            (-1 - @as(i32, @intCast(moonlet.opcodes.rkGetConstantIndex(@intCast(c)))))
                        else
                            c;
