...
fn checkArgMode(function: *const Function, val: i32, mode: OpCode.OpArgMask) !void {
    switch (mode) {
        ...
        .ConstantOrRegisterConstant => {
            ...
            const is_constant = moonlet.opcodes.rkIsConstant(actual_size_val);
            if (is_constant) {
                if (moonlet.opcodes.rkGetConstantIndex(actual_size_val) >= function.constants.len) return error.ConstantIndexOutOfRange;
            } else {
